jQuery(document).ready(function($){
	...
	$(".chartm-calend-round").click(function(){
		scrolled = window.pageYOffset || document.documentElement.scrollTop;
		ofs = $('#datepicker').offset().top;
		h = $(window).height();
		
		//h-calen = 220;
		
		alert(ofs+'--'+scrolled+'---'+h);
        ...
        $(function() {
            $("#datepicker").datepicker({
                ...
                onSelect: function(value, date) {
		            ...
		            //var url = path+'/search_cur.php';
		           // var url = 'http:/yii2b/dinamic-courses/index';
					//alert(value+'=='+valute);
					data_2 = getAjax('/dinamic-courses/index?date2='+value+'&valute2='+valute);
					data_2 = JSON.parse(data_2);
					
					$('#left-div2-td1').empty().append(data_2[0]);
					$('#left-div2-td2').empty().append(data_2[1]);
					$('#chartm-left-div1').hide();
					$('#chartm-left-div2').show();
					//$('#left-div2-td1').app();
					
					//alert(data_2[0]);
					//csrf = $('#csrf').val();
					/*alert(csrf);
					$.post(url,{'date2':value, 'valute2':valute, '_csrf':csrf},function(res){
						alert('asd');
			            $('#chartm-left-div1').css('display', 'none');
			            $('#chartm-left-div2').append('<table cellspacing="0" cellpadding="0" border="0"><tr><td class="pseudo-link-new1">'+value+'</td><td>'+res+'</td><td id="close-wind-cur">X</td></tr><table>');
			            $('#chartm-left-div2').css('padding', '10px');
                    });*/
                } 
            });
        });return false;
...
//-------------------------------
    $("#chartm-left-div2").on("click", ".pseudo-link-new1", function(event){	
		$(function() {
			$("#datepicker").datepicker({ 
				onSelect: function(value, date) {
					$( "#datepicker" ).datepicker( "destroy" );
...
//--------------------------------
    ...
    $("#chartm-left-div2").on("click", "#close-wind-cur", function(event){
		$('#chartm-left-div1').show();
		$('#chartm-left-div2').hide();
    });

    $(".cl-chart-a").click(function(){
		$('.cl-chart-a').css('background','white');
		$('.cl-chart-a').css('color','#0C82DF');
		$(this).css('background','#0C82DF');
		$(this).css('color','white');
		
		if($(this).attr('id') == 'chart-a1'){
			n=8;zoomChart(n);
			$(this).attr('background','green');
		}
		if($(this).attr('id') == 'chart-a2'){
			n=31;zoomChart(n);
		} 
		if($(this).attr('id') == 'chart-a3'){
			n=92;zoomChart(n);
		} 
		if($(this).attr('id') == 'chart-a4'){
			n=366;zoomChart(n);
		} 
		if($(this).attr('id') == 'chart-a5'){
			n=chart.dataProvider.length;zoomChart(n);
		} 
		return false;
    });

	$("#chartm-ref").click(function(){
		valute = $('#chartm-form-input').val();
		var url = path+'/update.php';
		$.post(url,{'valute':valute},function(res){
...
// chart -----------------------------------------------------------------
	...
	data2 = getAjax('/dinamic-courses/index?valute='+valute);
	//alert(data2);
   ...
   //-----------------------------------------------------------------------
	...
	var chart = AmCharts.makeChart("chartdiv", {
        "type": "serial",
		"pathToImages": "/web/images/dinamic-courses/",
        "theme": "light",
        "marginRight": 20,
		//"marginBottom": 20,
        "autoMarginOffset": 20,
        ...
        "graphs": [{
		    "id": "g1",
		    "negativeFillColors":"#AA0000",
            "bulletBorderAlpha": 1,
            "bulletColor": "#FF0000",
            "bulletSize": 5,//5
            "hideBulletsCount": 50,//50
            "lineThickness": 2,//2
            "title": "red line",
            "useLineColorForBulletBorder": true,
            "valueField": "value",
            "balloonText": "<div style='margin:5px; font-size:9px;'>[[value]] руб.</div>"
        }],
        "chartScrollbar": {
		    "graph": "g1",
		    "dragIcon": "drag",
            ...
            "offset":25,//30
            "scrollbarHeight": 50,//80
            "backgroundAlpha": 0,//0 цвет нижнего фона
			"selectedBackgroundAlpha": 1.0,//0.1
			"selectedBackgroundColor": "#CBCBCB",//сверху
			"selectedGraphFillAlpha": 1.0,
			"selectedGraphFillColor": "#868686",//снизу
			"graphFillAlpha": 1,//0 на две части верх и низ
			"graphLineAlpha": 1.0,//0.5
			"selectedGraphFillAlpha": 1,//0
			"selectedGraphLineAlpha": 0,//1
			"autoGridCount":true,//true
			"color":"#ffffff",
			"gridColor": "#FFFFFF",//цвет верт разделителя
			"gridAlpha": 1
		},
		"chartCursor": {
			"color":"#FFFFFF",
			"cursorColor":"#FF0000",//цвет ползунка
