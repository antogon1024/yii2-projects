jQuery(document).ready(function($) {
	...
	var sale_bay = 1;
	var select_bk = 'left'; // выбор сайта донора
...
//------- начальная загрузка
	...
	$.post(url,{path:path},function(res){ 
		...
		for(var i in pdata.banks){
			//$('#aconverter342-l').find('#ar-banks'+i).removeClass('empty-data');
			//alert(pdata.usd1[i]+'|||'+i);
			if(pdata.usd1[i] == '0' || pdata.usd1[i] == '-'){ 
				//alert(pdata.usd1[i]);
				str += '<div id="ar-banks'+i+'" class="empty-data">'+pdata.banks[i]+'</div>';
			 }else{
			...
			if(flag2 == 1){
				$('#converter342-label1').append(pdata.banks[i]);
				$('#converter342-hbank').val(pdata.banks[i]);//hidden
				setData(pdata, $('#converter342-label1').text());
				//alert($('#converter342-label1').text());
				flag2 = 2;
			}
        }
...
//------- выбор банка
	$('#converter342-label1').click(function() {
		if(select_bk == 'right')return false;
	...
	$('#converter342-banks').mouseleave(function(){
			...
			$("#datepicker").datepicker({
				...
				onSelect: function(value, date) {
					...
					$.post(url,{date:date},function(res){
						...
						$('#aconverter342-l div').removeClass('empty-data');
						flag2 = 0;
						for(var i in pdata.banks){
							if(pdata.usd1[i] == '0' || pdata.usd1[i] == '-'){
								$('#aconverter342-l #ar-banks'+i).addClass('empty-data');
							}else{
								if(flag2 != 2)
									flag2 = 1;
							}
